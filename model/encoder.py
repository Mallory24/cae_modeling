...
class CrossModalTrm(RobertaPreTrainedModel):
    ...
    def pad_vocab(self):    # TO CHECK: What is this for?
    ...
    def _compute_img_embeddings(self, img_feat, img_pos_ids,
        ...
        output = self.img_embeddings(img_feat,
                                     img_pos_ids,
                                     img_type_ids,
                                     img_masks)
        return output
    ...
    def init_type_embedding(self):
        new_emb = nn.Embedding(2, self.config.hidden_size)  # TO CHECK: why 2?
    ...
    def forward(self, batch, task='repr', compute_loss=True, get_hidden=False):
        ...
        if task == 'repr':
            ...
            c_sub_input_ids = batch['c_sub_input_ids']
            c_sub_pos_ids = batch['c_sub_pos_ids']
            c_v_feats = batch['c_v_feats']
            c_v_pos_ids = batch['c_v_pos_ids']
            c_v_type_ids = batch['c_v_type_ids']
            c_attn_masks = batch['c_attn_masks']
            c_gather_index = batch['c_level_gather_index']
            # handle mfm (frame mask)
            c_v_masks = batch['c_v_masks']
            return self.forward_repr(c_sub_input_ids, c_sub_pos_ids,
                                     c_v_feats, c_v_pos_ids,
                                     c_attn_masks, c_gather_index,
                                     img_type_ids=c_v_type_ids,
                                     img_masks=c_v_masks)
        elif task == 'txt':
            ...
            txt_mask_tgt = batch['txt_mask_tgt']
            txt_labels = batch['txt_labels']
            return self.forward_mam(input_ids, position_ids,
                                    img_feat, c_v_pos_ids,
                                    attention_mask, gather_index,
                                    txt_mask_tgt, txt_labels,
                                    c_v_type_ids,
                                    compute_loss, get_hidden)
    ...
    # MAM
    def forward_mam(self, input_ids, position_ids, img_feat, img_pos_ids,
                    attention_mask, gather_index, txt_mask_tgt,
                    txt_labels=None, img_type_ids=None,
                    compute_loss=True, get_hidden=False):
        embedding_output = self._compute_img_txt_embeddings(
            input_ids, position_ids, img_feat, img_pos_ids,
            gather_index, img_type_ids=img_type_ids)
        sequence_output = self.encoder(embedding_output, attention_mask)[0]
        ...
        if compute_loss:
            masked_lm_loss = F.cross_entropy(prediction_scores, txt_labels,
                                             reduction='none')
            if get_hidden:
                return masked_lm_loss, masked_output
            else:
                return masked_lm_loss
        else:
            if get_hidden:
                return prediction_scores, masked_output
            else:
                return prediction_scores
