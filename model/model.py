...
class HierarchicalVlModel(VideoPreTrainedModel):
    ...
    def forward(self, batch, task='repr', compute_loss=True, get_hidden=False):
        ...
        elif task.startswith('mam'):
            return self.f_encoder(batch, task, compute_loss, get_hidden)
        elif task == 'mffr':
            return self.forward_mfm(batch, compute_loss, loss='regression')
        elif task == 'mfm-nce':
            return self.forward_mfm(batch, compute_loss, loss='nce')
        elif task == 'mem-nce':
            return self.forward_mem(batch, compute_loss, loss='nce', single_sub=True)
        elif task == 'fom':
    ...
    def collect_clip_outputs(self, out_shape, frame_sequence_output,
                              num_frames):
        """
        Inputs:
        :out_shape              (bz, #frames, hidden_size)
        :frame_sequence_output  tensor (bz, max_vl+max_sl, hidden_size)
        ...
        for vid, num_frame in enumerate(num_frames):
            clip_sub = frame_sequence_output[vid]   # max_vl+max_sl x 768
            matched_v_feats[vid] = clip_sub[:num_frame]
    ...
    def forward_repr(self, batch, encode_clip=True, single_sub=False):
        c_v_feats = batch['c_v_feats']
        c_v_attn_masks = batch['c_v_attn_masks']
        
        if single_sub:
            # (bz, max_vl+max_sl, 768)
            frame_outputs = self.f_encoder(batch, task='clip_repr')
            frame_sequence_output = frame_outputs[0]
            
            num_frames = [i.size(0) for i in c_v_feats]

            # (bz, max_vl, 768)
            shape = list(c_v_feats.size()[:2]) + [frame_sequence_output.size(-1)]

            matched_v_feats = self.collect_clip_outputs(
                shape, frame_sequence_output, num_frames)
            
        else:
            # (total_sub, max_vl+max_sl, 768)
            # Q: total_subs = (bz of various lens of sub)?
            frame_outputs = self.f_encoder(batch, task='repr')
            frame_sequence_output = frame_outputs[0]
    ...
    def forward_mfm(self, batch, compute_loss=True, loss='regression'):
        ...
        # seems to produce the same effect as _mask_img_feat
        mask = self.mask_embedding(c_v_mask.long())
        c_v_feats_masked = c_v_feats + mask
        batch['c_v_feats'] = c_v_feats_masked
        clip_outputs = self.forward_repr(batch, encode_clip=True)
    ...
    def forward_mem(self, batch, compute_loss=True, loss='regression', single_sub=True, remove_same_video_AFT_frame=True):
        assert loss in ['regression', 'nce']
        c_v_feats = batch['c_v_feats']
        c_v_mask = batch['c_v_masks']
        # print("c_v_mask", c_v_mask)
        ...
        c_v_feats_masked = c_v_feats + mask
        batch['c_v_feats'] = c_v_feats_masked
        clip_outputs = self.forward_repr(batch, encode_clip=True, single_sub=single_sub)
        ...
        if compute_loss:
            ...
            if loss == 'regression':
                ...
                mfm_loss = self.mem_nce(prediction_feat,
                                        feat_targets, neg_pred_feat,
                                        compute_loss=True,
                                        remove_same_video_AFT_frame=remove_same_video_AFT_frame,
                                        c_v_masks=c_v_mask)
            return mfm_loss
        else:
            if loss == 'regression':
    ...
    def mfm_nce(self, masked_output, pos_output, neg_output,
        ...
        # dot product of negative samples
    ...
    def mem_nce(self, masked_output, pos_output, neg_output,
                compute_loss=True, remove_same_video_AFT_frame=True,
                compute_AFT_only=False, c_v_masks=None):
        
        # TODO: remove compute_AFT_only later
        # dot product of ground truth feature
        masked_score = masked_output.matmul(pos_output.t())
        
        # do not consider the [AFT] video frame within the same video as the frame negatives
        if remove_same_video_AFT_frame:
            masked_score = self._mask_score(masked_score, c_v_masks, value=-np.inf)
        
        if compute_AFT_only:
            # do not consider [BEF] [ACT] as the frame negatives
            logits = masked_score
        else:
            # consider [BEF] [ACT] as the frame negatives
            neg_score = masked_output.matmul(neg_output.t())
            logits = torch.cat([masked_score, neg_score], dim=1).float()
            # print("logits", logits)
            
        if compute_loss:
            # TODO: play around with self.nce_temp
            targets = torch.arange(0, masked_output.size(0),
                                   dtype=torch.long, device=logits.device)
            loss = F.cross_entropy(logits/self.nce_temp, targets,
                                   reduction='none')
    ...
    def _mask_score(self, score_matrix, mask, value=None):
        masked_score_mask = torch.zeros(score_matrix.size(), dtype=torch.bool, device=score_matrix.device)
        start_idx = 0
        frame_idx = 0
        for idx, mask in enumerate(mask):
            num_masked_frames = len(mask.nonzero())
            for i in range(num_masked_frames):
                # print('start index', start_idx)
                # print('frame index', frame_idx)
                # print('mask', mask)
                # mask left neighbors
                if i != 0:
                    masked_score_mask[frame_idx][start_idx:start_idx + i] = 1
                    # print('mask left neighbors', masked_score_mask)
                # mask right neighbors
                masked_score_mask[frame_idx][start_idx + i + 1: start_idx + num_masked_frames] = 1
                # print('mask right neighbors', masked_score_mask)
                frame_idx += 1
                
            start_idx += num_masked_frames

        score_matrix.masked_fill_(masked_score_mask, value)
    ...
    def initialize(self):
        ...
        self.c_encoder.apply(self.c_encoder.init_weights)   # TO CHECK: Do we include this?
...
class HeroModel(VideoPreTrainedModel):
    def __init__(self, config, vfeat_dim, max_frm_seq_len, mem_nce_temp):
        super().__init__(config)
        self.config = config
        self.v_encoder = HierarchicalVlModel(
            config, vfeat_dim, max_frm_seq_len, nce_temp=mem_nce_temp)
    ...
    def load_partial_pretrained(self, checkpoint, vfeat_dim, max_frm_seq_len,
        ...
        self.v_encoder.f_encoder.pad_vocab()    # TO CHECK: what is this for?
        self.v_encoder.init_type_embedding()  # TO CHECK: why become 2?