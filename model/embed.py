...
class ImageEmbeddings(nn.Module):
    def __init__(self, config, img_dim, max_img_seq_len):
        super().__init__()
        self.frame_type_vocab_size = config.frame_type_vocab_size
        ...
        self.token_type_embeddings = nn.Embedding(
            config.frame_type_vocab_size, config.hidden_size)
        # tf naming convention for layer norm
        self.LayerNorm = FusedLayerNorm(config.hidden_size, eps=1e-5)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, img_feat, img_pos_ids=None, token_type_ids=None,
                img_masks=None):
        device = img_feat.device if img_feat is not None\
            else img_feat.device
