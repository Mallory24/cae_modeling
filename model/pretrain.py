...
class HeroForPretraining(HeroModel):
    def __init__(self, config, vfeat_dim, max_frm_seq_len,
                 ...
                 hard_neg_weight=10, use_all_neg=True, mem_nce_temp=1.0):
        super().__init__(config, vfeat_dim, max_frm_seq_len, mem_nce_temp)
    ...
    def forward(self, batch, task='vsm', compute_loss=True, get_hidden=False):
        ...
        if task == 'vsm':
            ...
            return self.v_encoder(batch, 'mam', compute_loss, get_hidden)
        elif task == 'mffr':
            return self.v_encoder(batch, 'mffr', compute_loss)
        elif task == 'mfm-nce':
            return self.v_encoder(batch, 'mfm-nce', compute_loss)
        elif task == 'mem-nce':
            return self.v_encoder(batch, 'mem-nce', compute_loss)
        elif task == 'fom':
