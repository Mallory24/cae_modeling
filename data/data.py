...
class SubTokLmdb(TxtTokLmdb):
    def __init__(self, db_dir, max_clip_len=-1, single_sub=False):
...
class VideoFeatSingleSubTokDataset(Dataset):
    def __init__(self, txt_db, img_db, max_txt_len=60, sub_ctx_len=0):
        ...
        self.vids = self.img_db.name2nframe
    ...
    def __getitem__(self, vid_):
        '''
        clip_level_input_ids
        v_feat(frames)
        clip_level_attn_masks
        clip_level_v_attn_masks
        '''
        example = self.txt_db[vid_]
        v_feat, nframes = self._get_v_feat(vid_)

        # clip_level_input_ids = [self.txt_db.sep] + []
        clip_level_input_ids = copy.deepcopy(example['input_id'])
        clip_level_input_ids = torch.tensor(clip_level_input_ids)

        clip_level_v_feats = v_feat
        clip_level_attn_masks = [1] * (len(clip_level_v_feats) + len(clip_level_input_ids))
        clip_level_attn_masks = torch.tensor(clip_level_attn_masks)
        
        # For temporal encoding of video frames
        clip_level_v_attn_masks = [1] * len(clip_level_v_feats)
        clip_level_v_attn_masks = torch.tensor(clip_level_v_attn_masks)
        
        out = (clip_level_input_ids,    # 1 list[tensor(sep,w0,w1,...)]
               clip_level_v_feats,      # tensor(num_frames, d)
               clip_level_attn_masks,   # 1 tensor[1 * (L_sub + #sub_frames)]
               clip_level_v_attn_masks) # 1 tensor[1 * (#sub_frames)]
        return out
...
def video_collate_clip_level(inputs):
    (clip_level_input_ids,
     clip_level_v_feats,
     clip_level_v_type_ids,
     clip_level_attn_masks,
     clip_level_v_attn_masks) = map(list, unzip(inputs))

    txt_lens = [i.size(0) for i in clip_level_input_ids]
    num_frames = [i.size(0) for i in clip_level_v_feats]

    # hard_coded padding value, TODO: check correctness # TO CHECK
    clip_level_input_ids = pad_sequence(
        clip_level_input_ids, batch_first=True, padding_value=1)
    
    clip_level_sub_pos_ids = torch.arange(
        0, clip_level_input_ids.size(1), dtype=torch.long
    ).unsqueeze(0).expand(clip_level_input_ids.size(0), -1).clone()
    ...
    clip_level_v_attn_masks = pad_sequence(
            clip_level_v_attn_masks, batch_first=True, padding_value=0)
    
    bs, max_vl, _ = clip_level_v_feats.size()
    out_size = clip_level_attn_masks.size(1)    # out_size: max_vl+sl
    clip_level_gather_index = get_gather_index(txt_lens, num_frames, bs, max_vl, out_size)

    batch = {'c_sub_input_ids': clip_level_input_ids,  # (bz, max_sl)
             'c_sub_pos_ids':  clip_level_sub_pos_ids,  # (bz, max_sl)
             'c_v_feats': clip_level_v_feats,         # (bz, max_vl, k)
             'c_v_type_ids': clip_level_v_type_ids,   # (bz, max_vl)
             'c_v_pos_ids': clip_level_pos_ids,         # (bz, max_vl)
             'c_attn_masks': clip_level_attn_masks,     # (bz, max_vl+sl)
             'c_v_attn_masks': clip_level_v_attn_masks,  # (bz, max_vl)
             'c_level_gather_index': clip_level_gather_index}  # (bz, max_vl+sl)
    return batch
...
def get_gather_index(txt_lens, num_frames, batch_size, max_len, out_size):
    ...
    # print("gather_index size", gather_index.size())
