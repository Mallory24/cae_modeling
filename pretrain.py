...
def build_target_loaders(target, tgt_ratio, opts):
    if 'vfeat_shards' in target:
        ...
        video_db = load_video_sub_dataset(
            ...
            target['vfeat_interval'], opts)
        opts.vfeat_interval = target['vfeat_interval']
    train_loaders = {}
    val_loaders = {}
    for split in target['splits']:
        ...
        for task, r in zip(split['tasks'], split['ratio']):
            name = f"{task}_{target['name']}_{split['name']}"   # EXP: mam_cae_all
            ...
            if isinstance(video_db, list):
                ...
                train_ids = json.load(
                    open(f"{opts.txt_db}/{target['sub_txt_db']}/{split['train_idx']}"))
            val_ids = json.load(open(f"{opts.txt_db}/{target['sub_txt_db']}/{split['val_idx']}"))
            if task == 'mlm':
                if isinstance(video_db, list):
                ...
                if isinstance(video_db, list):
                    train_dset = ConcatDataset([
                        VideoMamDataset(ids, vid_db, opts.mask_prob,
                                        sub_ctx_len=opts.sub_ctx_len,
                                        mask_strategy=opts.mask_strategy,
                                        subtitle_only=opts.subtitle_only,
                                        without_aft=opts.without_aft)
                        for ids, vid_db in zip(all_train_ids, video_db)
                    ])
                    val_dset = VideoMamDataset(
                        val_ids, video_db[0], opts.mask_prob,
                        sub_ctx_len=opts.sub_ctx_len,
                        mask_strategy='verb_only',
                        subtitle_only=opts.subtitle_only,
                        without_aft=opts.without_aft)
                else:
                    train_dset = VideoMamDataset(
                        train_ids, video_db, opts.mask_prob,
                        sub_ctx_len=opts.sub_ctx_len,
                        mask_strategy=opts.mask_strategy,
                        subtitle_only=opts.subtitle_only,
                    ...
                    val_dset = VideoMamDataset(
                        ...
                        subtitle_only=opts.subtitle_only,
                        without_aft=opts.without_aft)
                train_collate = mam_collate
                val_collate = mam_collate
                
            elif task == 'mem-nce':
                 # TODO: remove opts.frame_mask_strategy?
                if opts.train_frame_negative_type in ['object_based', 'video_based']:
                    train_dset = MemDataset(train_ids, video_db,
                                            opts.mask_prob, opts.frame_type_vocab_size,
                                            opts.frame_mask_strategy, opts.video_frame_only, default_sampler=False)
                    train_neg = json.load(
                        open(f"{opts.txt_db}/{target['sub_txt_db']}/{target['frame_negative']['train_neg']}"))
                    train_sampler = MemNegativeSampler(train_neg, train_ids, opts.train_batch_size, "train")
                
                elif opts.train_frame_negative_type == 'randomized':
                    train_dset = MemDataset(train_ids, video_db,
                                            opts.mask_prob, opts.frame_type_vocab_size,
                                            opts.frame_mask_strategy, opts.video_frame_only)
                    
                if opts.val_frame_negative_type in ['object_based', 'video_based']:
                    val_dset = MemDataset(val_ids, video_db,
                                          opts.mask_prob, opts.frame_type_vocab_size,
                                          opts.frame_mask_strategy, opts.video_frame_only, default_sampler=False)
                    val_neg = json.load(
                        open(f"{opts.txt_db}/{target['sub_txt_db']}/{target['frame_negative']['val_neg']}"))
                    val_sampler = MemNegativeSampler(val_neg, val_ids, opts.val_batch_size, "val")
                    
                elif opts.val_frame_negative_type == 'randomized':
                    val_dset = MemDataset(val_ids, video_db,
                                          opts.mask_prob, opts.frame_type_vocab_size,
                                          opts.frame_mask_strategy, opts.video_frame_only)

                train_collate = mem_collate
...
def main(opts):
    ...
    meta_loader = MetaLoader(train_dataloaders,
                             accum_steps=opts.gradient_accumulation_steps,  # 2
                             distributed=n_gpu > 1,
                             round_robin=opts.round_robin)
    ...
    if opts.load_partial_pretrained:
        ...
        model = HeroForPretraining(
            ...
            lw_neg_ctx=opts.lw_neg_ctx, # 8.0
            lw_neg_q=opts.lw_neg_q, lw_st_ed=0, # 8.0
            ...
            mem_nce_temp=opts.mem_nce_temp)
    ...
    # print the amount of total trainable params
    ...
    best_valid_stats = {f'{task}': {'acc': float(0), 'loss': float('inf')} for task in train_dataloaders.keys()}
    ...
    for step, (task, batch) in enumerate(meta_loader):
        ...
        if (step + 1) % opts.gradient_accumulation_steps == 0:
            ...
            if global_step % opts.valid_steps == 0:
                ...
                LOGGER.info(f"Step {global_step}: train loss {loss}")
                val_log = validate(model, val_dataloaders, opts)

                if opts.checkpoint_saving_strategy == 'acc':
                    joint_acc_better = 0
                    for task in val_log.keys():
                        if val_log[task]['acc'] > best_valid_stats[task]['acc']:
                            best_valid_stats[task]['acc'] = val_log[task]['acc']
                            joint_acc_better += 1
                            
                    # if the accuracies of all the pretraining tasks increase
                    if joint_acc_better == len(val_log):
                        LOGGER.info(f"Step {global_step}: save model with the best valid acc {best_valid_stats}")
                        model_saver.save(model, global_step)

                elif opts.checkpoint_saving_strategy == 'loss':
                    joint_loss_better = 0
                    for task in val_log.keys():
                        if val_log[task]['loss'] < best_valid_stats[task]['loss']:
                            best_valid_stats[task]['loss'] = val_log[task]['loss']
                            joint_loss_better += 1
                    
                    # if the losses of all the pretraining tasks decrease
                    if joint_loss_better == len(val_log):
                        LOGGER.info(f"Step {global_step}: save model with the best valid loss {best_valid_stats}")
                        model_saver.save(model, global_step)
                LOGGER.info('===========================================')
    ...
    if global_step % opts.valid_steps != 0:
        ...
        LOGGER.info('===========================================')
        _ = model_saver.save(model, global_step)
...
def validate(model, val_dataloaders, opts):
    ...
    for task, loader in val_dataloaders.items():
        ...
        if task.startswith('mlm'):
            ...
            val_log = validate_mam(model, loader)
        ...
        elif task.startswith('mem-nce'):
            val_log = validate_mem_nce(model, loader)
...
@torch.no_grad()
def validate_mam(model, val_loader):
    LOGGER.info("start running MAM validation...")
    val_loss = 0
    n_correct = 0
    n_word = 0
    st = time()
...
@torch.no_grad()
def validate_mem_nce(model, val_loader):
    LOGGER.info("start running MEM-NCE validation...")
    val_loss = 0
    val_l2 = 0
    n_correct = 0
    cosine = 0
