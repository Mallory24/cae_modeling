...
from data import (
    ...
    VideoFeatSubTokDataset, VideoFeatSingleSubTokDataset, VideoFeatDataset,
...
def load_video_sub_dataset(v_feat_path, sub_txt_db, vfeat_interval, opts):
    ...
    if not isinstance(sub_txt_db, SubTokLmdb):
        if hasattr(opts, "task") and "msrvtt" in opts.task:
            ...
            sub_txt_db = SubTokLmdb(sub_txt_db, opts.max_clip_len, opts.single_sub)
            
    if opts.single_sub:
        video_db = VideoFeatSingleSubTokDataset(
            sub_txt_db, vfeat_db,
